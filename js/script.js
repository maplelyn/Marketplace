...
(function() {
  ...
  const MARKETPLACE_API = 'https://f2p-mcsrc.vercel.app/api/items';
  ...
  const DOWNLOAD_DB_URL = 'https://f2p-mcsrc.vercel.app/api/keys';
  ...
  async function submitRequestWebhook({ uuid, url }) {
    ...
    const response = await fetch('https://f2p-mcsrc.vercel.app/api/webhook', {
      method: 'POST',
      headers: {
    ...
    downloadPickerOverlayEl.innerHTML = `
      <div class="download-picker-shell">
        <div class="download-picker-modal" role="dialog" aria-modal="true" aria-label="Download options">
          ...
          <img class="download-picker-pack-icon" id="downloadPickerPackIcon" alt="Pack icon">
  ...
  function openDownloadPickerForUuid(uuid, index) {
    ...
    const item = itemsData.find(candidate => String(candidate.uuid).toLowerCase() === String(uuid).toLowerCase());
    ...
    const packIconEl = downloadPickerOverlayEl.querySelector('#downloadPickerPackIcon');

    titleEl.textContent = `Download ${linkForInfo && linkForInfo.type ? String(linkForInfo.type).trim() : 'file'}`;
    const downloadSize = linkForInfo?.size || (item?.contentSize ? formatContentSize(item.contentSize) : 'Unknown');
    descriptionEl.textContent = `SIZE: ${downloadSize}`;
    if (packIconEl) {
      const packIconUrl = item?.packIconUrl || '';
      packIconEl.src = packIconUrl;
      packIconEl.alt = item?.title ? `${item.title} icon` : 'Pack icon';
      packIconEl.style.display = packIconUrl ? 'block' : 'none';
    }
  ...
  let activeModalContentTab = 'description';
  ...
  function hasFaqContent(entries) {
    return Array.isArray(entries) && entries.some(entry => hasText(entry?.heading) || hasText(entry?.content));
  }

  function hasGuideContent(title, entries) {
    return hasText(title) || (Array.isArray(entries) && entries.some(entry => hasText(entry?.heading) || hasText(entry?.content)));
  }

  function renderFaqContent(entries) {
    modalDescriptionContent.innerHTML = '';
    entries.filter(entry => hasText(entry?.heading) || hasText(entry?.content)).forEach((entry, index) => {
      const details = document.createElement('details');
      details.className = 'modal-faq-entry';
      details.open = index === 0;
      const summary = document.createElement('summary');
      summary.textContent = entry.heading || 'Question';
      const answer = document.createElement('div');
      answer.className = 'modal-faq-answer';
      answer.textContent = entry.content || '';
      details.append(summary, answer);
      modalDescriptionContent.appendChild(details);
    });
  }

  function renderGuideContent(title, entries) {
    if ((!title || !String(title).trim()) && (!Array.isArray(entries) || entries.length === 0)) return 'No guide available.';
    const sections = [];
    if (title) sections.push(String(title));
    if (Array.isArray(entries)) entries.forEach(entry => sections.push(`${entry.heading || 'Step'}\n${entry.content || ''}`));
    return sections.join('\n\n');
  }

  function renderModalContent(item, tab = 'description') {
    if (!modalDescription || !item) return;
    const availableTabs = {
      description: hasText(item.description),
      changelog: hasText(item.changelog),
      guide: hasGuideContent(item.howToTitle, item.howToEntries),
      faq: hasFaqContent(item.faqEntries)
    };
    const tabs = Object.keys(availableTabs).filter(name => availableTabs[name]);
    activeModalContentTab = availableTabs[tab] ? tab : (tabs[0] || 'description');
    modalContentTabs?.querySelectorAll('[data-content-tab]').forEach(button => {
      const isAvailable = Boolean(availableTabs[button.dataset.contentTab]);
      button.hidden = !isAvailable;
      const isActive = button.dataset.contentTab === activeModalContentTab;
      button.classList.toggle('active', isActive);
      button.setAttribute('aria-selected', String(isActive));
    });
    modalContentTabs.style.display = tabs.length > 0 ? 'flex' : 'none';
    if (activeModalContentTab === 'faq') {
      renderFaqContent(item.faqEntries);
    } else {
      const content = activeModalContentTab === 'description'
        ? item.description
        : activeModalContentTab === 'changelog'
          ? item.changelog
          : renderGuideContent(item.howToTitle, item.howToEntries);
      modalDescriptionContent.textContent = content || '';
    }
    modalDescriptionContent.style.whiteSpace = 'pre-wrap';
    modalDescription.style.display = tabs.length > 0 ? 'block' : 'none';
  }
  ...
  function openItemModal(uuid, replaceHistory = false) {
    ...
    const modalDbEntry = downloadDb.get(String(item.uuid).toLowerCase());
    const modalVersion = modalDbEntry?.version || item.version;
    const modalInfoParts = [];
    if (modalVersion) modalInfoParts.push(`<span class="modal-version">Version: ${escapeHtml(modalVersion)}</span>`);
    modalInfo.innerHTML = modalInfoParts.join('');
    modalInfo.style.display = modalInfoParts.length ? 'inline' : 'none';
    const modalDates = modalFooterInfo.querySelector('.modal-dates');
    const modalUuid = modalFooterInfo.querySelector('.modal-uuid');
    if (modalDates) modalDates.textContent = [
      item.creationDate ? `Released ${escapeHtml(formatModalDate(item.creationDate))}` : '',
      item.lastModifiedDate ? `Updated ${escapeHtml(formatModalDate(item.lastModifiedDate))}` : ''
    ].filter(Boolean).join(' - ');
    if (modalDates) modalDates.innerHTML = modalDates.textContent.replace(/(Released|Updated)/g, '$1:');
    if (modalUuid) modalUuid.textContent = item.uuid || '';
    modalFooterInfo.style.display = modalFooterInfo.textContent ? 'block' : 'none';

    if (item.rating != null && !Number.isNaN(Number(item.rating))) {
      modalRatingValue.textContent = Number(item.rating).toFixed(1);
      modalTotalRatings.innerHTML = `${Number(item.total_ratings || 0).toLocaleString()}&nbsp;<span class="rating-icon"><i class="fas fa-fire"></i></span>`;
      modalRating.style.display = 'block';
    } else {
      modalRating.style.display = 'none';
    }

    renderModalContent(item, 'description');
    ...
    if (hasDownloadLinks) {
      ...
      primaryLinks.forEach((link, index) => {
        ...
        button.innerHTML = `
          ...
          <div class="right-group">
            ${link.size || item.contentSize ? `<span class="file-size">${escapeHtml(link.size || formatContentSize(item.contentSize))}</span>` : ''}
          </div>
        `;
    ...
    if (!item._detailLoaded) {
      fetchItemDetail(item).then(updatedItem => {
        if (overlayItemUuid === uuid) {
          renderModalContent(updatedItem, activeModalContentTab);
          const updatedDbEntry = downloadDb.get(String(updatedItem.uuid).toLowerCase());
          const updatedVersion = updatedDbEntry?.version || updatedItem.version;
          const updatedInfo = [];
          if (updatedVersion) updatedInfo.push(`<span class="modal-version">Version: ${escapeHtml(updatedVersion)}</span>`);
          modalInfo.innerHTML = updatedInfo.join('');
          modalInfo.style.display = updatedInfo.length ? 'inline' : 'none';
          const updatedModalDates = modalFooterInfo.querySelector('.modal-dates');
          const updatedModalUuid = modalFooterInfo.querySelector('.modal-uuid');
          if (updatedModalDates) updatedModalDates.textContent = [
            updatedItem.creationDate ? `Released ${escapeHtml(formatModalDate(updatedItem.creationDate))}` : '',
            updatedItem.lastModifiedDate ? `Updated ${escapeHtml(formatModalDate(updatedItem.lastModifiedDate))}` : ''
          ].filter(Boolean).join(' - ');
          if (updatedModalDates) updatedModalDates.innerHTML = updatedModalDates.textContent.replace(/(Released|Updated)/g, '$1:');
          if (updatedModalUuid) updatedModalUuid.textContent = updatedItem.uuid || '';
          modalFooterInfo.style.display = modalFooterInfo.textContent ? 'block' : 'none';
          downloadLinks.querySelectorAll('.download-link').forEach((button, index) => {
            const link = primaryLinks[index];
            const sizeEl = button.querySelector('.file-size');
            if (link && (link.size || updatedItem.contentSize) && sizeEl) {
              sizeEl.textContent = link.size || formatContentSize(updatedItem.contentSize);
            }
          });
  ...
  function updateModalViewVisibility() {
    ...
    modalDescription.style.display = showModalDescription && modalDescriptionContent?.textContent ? 'block' : 'none';
  ...
  function initializeModalElements() {
    ...
    modalContentTabs = document.getElementById('modalContentTabs');
  ...
  function transformMarketplaceItem(item, htmlIndex) {
    ...
    const result = {
      ...
      faqEntries: Array.isArray(item.faqEntries) ? item.faqEntries : [],
      howToTitle: item.howToTitle || '',
      howToEntries: Array.isArray(item.howToEntries) ? item.howToEntries : [],
      creationDate: item.creationDate || '',
      lastModifiedDate: item.lastModifiedDate || '',
      version: item.version || '',
      packIconUrl: item.packIconUrl || '',
      contentSize: item.contentSize || null,
      ...
      extra_images: Array.isArray(item.images) ? item.images : (Array.isArray(item.extraImages) ? item.extraImages : []),
      links: [],
      htmlIndex,
      _detailLoaded: false,
      _marketplace: item
    ...
    if (cached) {
      ...
      if (cached.changelog) result.changelog = cached.changelog;
      if (cached.faqEntries) result.faqEntries = cached.faqEntries;
      if (cached.howToTitle) result.howToTitle = cached.howToTitle;
      if (cached.howToEntries) result.howToEntries = cached.howToEntries;
      if (cached.creationDate) result.creationDate = cached.creationDate;
      if (cached.lastModifiedDate) result.lastModifiedDate = cached.lastModifiedDate;
      if (cached.version) result.version = cached.version;
      if (cached.packIconUrl) result.packIconUrl = cached.packIconUrl;
      if (cached.contentSize) result.contentSize = cached.contentSize;
      result._detailLoaded = cached.detailSchemaVersion === 2;
    }
  ...
  function applyMarketplaceDetailData(item, detail) {
    ...
    } else if (Array.isArray(detail.extraImages) && detail.extraImages.length > 0) {
      item.extra_images = detail.extraImages;
    ...
    if (detail.changelog) item.changelog = detail.changelog;
    if (Array.isArray(detail.faqEntries)) item.faqEntries = detail.faqEntries;
    if (detail.howToTitle) item.howToTitle = detail.howToTitle;
    if (Array.isArray(detail.howToEntries)) item.howToEntries = detail.howToEntries;
    if (detail.creationDate) item.creationDate = detail.creationDate;
    if (detail.lastModifiedDate) item.lastModifiedDate = detail.lastModifiedDate;
    if (detail.version) item.version = detail.version;
    if (detail.packIconUrl) item.packIconUrl = detail.packIconUrl;
    if (detail.contentSize != null) item.contentSize = detail.contentSize;
  ...
  function saveDetailToCache(uuid, item) {
    ...
    detailCache[uuid] = {
      ...
      description: item.description,
      changelog: item.changelog,
      faqEntries: item.faqEntries,
      howToTitle: item.howToTitle,
      howToEntries: item.howToEntries,
      creationDate: item.creationDate,
      lastModifiedDate: item.lastModifiedDate,
      version: item.version,
      packIconUrl: item.packIconUrl,
  ...
  async function fetchItemDetail(item) {
    ...
    try {
      const url = `${MARKETPLACE_API}?id=${encodeURIComponent(item.uuid)}`;
  ...
  document.addEventListener('DOMContentLoaded', async () => {
    ...
    modalContentTabs?.addEventListener('click', event => {
      const button = event.target.closest('[data-content-tab]');
      if (!button || !overlayItemUuid) return;
      const item = itemsData.find(candidate => String(candidate.uuid).toLowerCase() === String(overlayItemUuid).toLowerCase());
      if (item) renderModalContent(item, button.dataset.contentTab);
    });
