...
(function() {
  ...
  async function getRequestAuthToken() {
    const now = Date.now();
    if (cachedRequestToken && now < cachedRequestTokenExpiry) return cachedRequestToken;
    ...
    if (!response.ok) throw new Error(`Failed to fetch request token: ${response.status}`);

    const data = await response.json();
    const token = data && data.token ? String(data.token) : '';
    if (!token) throw new Error('Request token missing from server response');
